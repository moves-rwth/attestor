		...
		while(!vertexQueue.isEmpty()){
			...
			if(!currentVertex.getFormulae().isEmpty()){
				...
				if(isNextForm(currentSubformula)){
					...
					Set<ProgramState> successors = new HashSet<ProgramState>();
					// Collect the "real" successor states (i.e. skipping materialisation steps)
					if(statespace.getMaterializationSuccessorsOf(currentState).size() != 0){
						for(ProgramState matState : statespace.getMaterializationSuccessorsOf(currentState)){
							// Every materialisation state is followed by a control flow state
							assert statespace.getControlFlowSuccessorsOf(matState).size() != 0;
							successors.addAll(statespace.getControlFlowSuccessorsOf(matState));
						}
					} else {
						successors.addAll(statespace.getControlFlowSuccessorsOf(currentState));
						// In case the state is final
						successors.addAll(statespace.getArtificialInfPathsSuccessorsOf(currentState));
					}
